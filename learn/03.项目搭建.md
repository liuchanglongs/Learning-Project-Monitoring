...
- 用户执行 pnpm commit ：
  - 这会启动 commitizen 工具，结合 cz-git 适配器提供交互式界面
...
- Git 提交信息验证（commit-msg 钩子）：git hooks 触发 @commitlint/cli 验证 commit 信息
  - 【注意】根据当前项目配置， 这个环节缺失 ，因为 .husky 目录中没有 commit-msg 钩子文件
  - 如果配置了 commit-msg 钩子，它会触发 @commitlint/cli 来验证 commit 信息
